...
export default function InvoiceAll({ params }: InvoiceFormProps) {
    ...
    const fetchInvoices = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          const { patient, Invoice } = response.data.data;
  ...
  const generatePDF = (item: Invoice[], patientData: Patient, invoice: any) => {
    ...
    doc.setTextColor(15, 12, 2); // Light Red
    doc.text("Dr Tedla Specialized Dental Clinic", 109, 24, { align: "center" });
    doc.setFontSize(18);
    ...
    doc.setDrawColor(15, 12, 2);
    doc.setLineWidth(1.5);
    doc.line(10, 40, 200, 40);
  
    doc.setDrawColor(89, 154, 222); // Light Secondary Line
    ...
    // Left Side Information
    doc.text(`From: Tedla Tessema Gambura`, 10, startY);
    doc.text(`Address: A.A W. 08 H.N: 216`, 10, startY + 7);
    doc.text(`Supplier's TIN No: 0037339391`, 10, startY + 14);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    doc.text("Thank you for choosing Dr Tedla Specialized Dental Clinic", 105, footerY + 5, {
      align: "center",
    });
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text("Contact: 091 333 2731 | 011 827 5888", 105, footerY + 10, {
      align: "center",
    });
    doc.text(
      "Address: Lideta sub-city Wereda 08 AIA Building H.No-new/216 Addis Ababa, Ethiopia",
      105,
      footerY + 15,
      { align: "center" }
    );
