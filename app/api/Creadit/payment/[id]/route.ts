...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const patient = await Patient.findById(id).populate('Credit').exec();
    if (!patient) {
      return NextResponse.json({ error: "Patient not found" }, { status: 404 });
    }

    // If the patient has no Credit, return an empty array
    if (!patient.Credit || patient.Credit.length === 0) {
      return NextResponse.json({ message: "No Credit available for this patient", data: [] });
    }

    // Sort Credit by createdAt field in descending order
    const sortedCredit = patient.Credit.sort((a: CreditDocument, b: CreditDocument) => {
