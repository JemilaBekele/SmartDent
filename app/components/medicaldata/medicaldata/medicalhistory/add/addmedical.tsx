...
type FormData = {
  ...
  TreatmentPlan: TreatmentDetails[]; // Fixed to be an array
  TreatmentDone: TreatmentDetails[];
  diseases: string[]; // Fixed to be an array
};
...
export default function MedicalFindingForm({ params }: MedicalFindingFormProps) {
  ...
  const validateForm = () => {
    const newErrors = {
      ChiefCompliance: formData.ChiefCompliance ? "" : "Chief Complaint is required.",
      Historypresent: formData.Historypresent ? "" : "History Present is required.",
      DrugAllergy: formData.DrugAllergy ? "" : "Drug Allergy is required.",
      Diagnosis: formData.Diagnosis?"": "Diagnosis is required",
      ExtraoralExamination:formData.ExtraoralExamination?"":"Extraoral Examination is required ",
      IntraoralExamination:formData.IntraoralExamination?"":"Intraoral Examination is required "
    };
  ...
  const handleAddEntry = (field: "TreatmentPlan" | "TreatmentDone") => {
    setFormData((prevData) => ({
      ...
      [field]: [
        ...(prevData[field] || []),
        { ToothNumber: "", Plan: "", Done: "",other: "" }, // Default object
      ],
    }));
  };
      ...
      <div className="flex-grow md:ml-60 container mx-auto p-4">
        <div className="flex space-x-8">
          ...
          <div className="w-2/3 bg-white p-6 rounded-lg shadow-md">
            <h1 className="text-2xl font-bold mb-4">Medical Findings</h1>
              ...
              <div className="flex-grow px-4">
                ...
                {showAll && (
                  <>
                    ...
                    {finding.createdAt && <p>Created at:
                    {new Date(finding.createdAt|| "") .toLocaleString()}</p>}
                    {finding.updatedAt && <p>Updated at:
                      {new Date(finding.updatedAt|| "") .toLocaleString()} </p>}
                  </>
                )}
...
type="button"  
              ...
              <InputField
                ...
                isTextArea={true}
                rows={3}
                error={errors.IntraoralExamination}
              />
              <InputField
                ...
                isTextArea={true}
                rows={3}
                error={errors.ExtraoralExamination}
              />
              <InputField
                label="Investigation"
                id="Investigation"
